...
export function NavLink({ href, icon, label, isActive, onClick }: NavLinkProps) {
  return (
    <Link
      ...
      onClick={onClick}
      className={`
        group flex items-center gap-[var(--space-3)] px-[var(--space-3)] py-[var(--space-2)]
        rounded-[var(--radius-md)] text-[var(--text-sm)] font-normal
    ...
    >
      <span
        className={`
          ...
          ${isActive ? "text-[var(--color-primary)]" : "text-[var(--color-neutral-500)] group-hover:text-[var(--color-neutral-300)]"}
        `}
      >
        {icon}
      </span>
